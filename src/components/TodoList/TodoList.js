...
export default class TodoList extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      lastClickedDate: new Date().toDateString(),
      todos: []
    };
  }
  ...
  render() {
    ...
    return (
      <section className='todos-container-layout'>
        <h2>TodoList</h2>
        <span>Current Date: {this.state.lastClickedDate.toString()}</span>
        ...
        <ul className='todos-container'>
          {
            this.state.todos
              .filter(todo =>
                ...
                new Date(this.state.lastClickedDate).toDateString())
              .map(todo => (
                <Todo
                  todo={todo}
                  key={todo.id}
